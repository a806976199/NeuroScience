...
\subsection{Independent-Spike, Independent-\\
  Neuron, and Correlation Codes}
\rem All information in this section refers to stimulating information.
\defn A code based solely on the time-dependent firing rate is called the \emph{independent-spike code}. This refers to the fact that the generation of each spike is independent of all the other spikes in the train.
\defn Individual spikes do not encode independently of each other, correlations between spike times may carry additional correlation code information, which is called the \emph{correlation codes}.
...
\defn The \emph{independent-neuron code} means the response of each neuron in a neural population is considered statistically independent.
